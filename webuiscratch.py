...
#scratch
parser.add_argument("--starget", type=str, default="txt2img")
parser.add_argument("--sprompt", type=str, default="a beautiful waluigi")
parser.add_argument("--ssteps", type=int, default=50)
parser.add_argument("--stoggles", type=int, nargs="*", default = [1, 2, 3, 4, 5])
parser.add_argument("--ssampler", type=str, default="k_lms")
parser.add_argument("--sddimeta", type=float, default=0.0)
parser.add_argument("--sbatches", type=int, default = 1)
parser.add_argument("--sbsize", type=int, default = 1)
parser.add_argument("--scfgscale", type=float, default =7.5)
parser.add_argument("--sseed", type=int, default=None)
parser.add_argument("--sh", type=int, default=512)
parser.add_argument("--sw", type=int, default=512)
parser.add_argument("--b_rows", type=int, default=0, help="rows in testgrid; use -1 for autodetect and 0 for one row per prompt")
...
#        kwargs = yaml.safe_load(f)
    ...
    scratchargs={
        #'prompt':opt.sprompt,
        'ddim_steps':opt.ssteps,
        'toggles':opt.stoggles,
        'sampler_name':opt.ssampler,
        'ddim_eta':opt.sddimeta,
        'n_iter':opt.sbatches,
        'batch_size':opt.sbsize,
        'cfg_scale':opt.scfgscale,
        'seed':opt.sseed,
        'height':opt.sh,
        'width':opt.sw,
        'fp':None,
    ...
    for i, prompt_i in enumerate(prompts):
        print(f"===== Prompt {i+1}/{len(prompts)}: {prompt_i} =====")
        output_images, seed, info, stats = target_func(prompt=prompt_i, **scratchargs)
        print(f'Seed: {seed}')
        print(info)
        print(stats)
        print()
    
def run_headless_bscratch():
    with open(opt.cli, 'r', encoding='utf8') as f:
        kwargs = yaml.safe_load(f)
    target = kwargs.pop('target')
    if target == 'txt2img':
    ...
    cfg_scale = cfg_scale if type(cfg_scale) is list else [cfg_scale]
    
    prompt = kwargs.pop("prompt")
    prompt = prompt if type(prompt) is list else [prompt]
    
    seed = kwargs.pop("seed")
    seed = seed if type(seed) is list else [seed]
    if seed[0] == None:
        seed[0] = random.randint(-32767, 32767)
    for s, seed_s in enumerate(seed):
        if seed_s == "RAND":
            seed[s] = random.randint(-32767, 32767)
    
    sampler_name = kwargs.pop("sampler_name")
    sampler_name = sampler_name if type(sampler_name) is list else [sampler_name]
    
    ddim_steps = kwargs.pop("ddim_steps")
    ddim_steps = ddim_steps if type(ddim_steps) is list else [ddim_steps]
    
    loopables = ['prompt', 'seed', 'sampler_name', 'cfg_scale', 'ddim_steps']
    
    testorder = kwargs.pop("testorder")
    if testorder == None:
        testorder = []
    
    for l, tl in enumerate(loopables):
        if tl not in testorder:
            testorder.append(tl)
            print(f"appended {tl}.")
    
    for l, testorder_l in enumerate(testorder):
        loops[l] = locals()[testorder_l]
        print(f"loop #{l+1} is {testorder_l}, starting with {loops[l][0]}")
    
    for l, l_l in enumerate(loops):
        if l_l == 0:
            loops[l]=[0]
    #print(f"loops: {loops}")
    ...
    for l in loops:
        pnum = pnum * len(l)
    ng = pnum / (len(loops[0])*len(loops[1]))
    print (" ")
    print(f"     ~~~~ I expect to generate {pnum} samples in {int(ng)} grids. ~~~~~")
    print (" ")
    num = 0
    
    for z, l_z in enumerate(loops[4]):
        if testorder[4] != None:
        
            if type(l_z) == str:
                e_exp4 = e_exp5 + f"{testorder[4]}=\"{l_z}\", "
            else:
                e_exp4 = e_exp5 + f"{testorder[4]}={l_z}, "
                
            #print(f"==== Setting {testorder[4]} to {l_z}, #{z+1} of {len(loops[4])}. ====")    
        for a, l_a in enumerate(loops[3]):
            if testorder[3] != None:
            
                if type(l_a) == str:
                    e_exp3 = e_exp4 + f"{testorder[3]}=\"{l_a}\", "
                else:
                    e_exp3 = e_exp4 + f"{testorder[3]}={l_a}, "
                    
                #print(f"==== Setting {testorder[3]} to {l_a}, #{a+1} of {len(loops[3])}. ====")
                
            for b, l_b in enumerate(loops[2]):        
                if testorder[2] != None:
                
                    if type(l_b) == str:
                        e_exp2 = e_exp3 + f"{testorder[2]}=\"{l_b}\", "
                    else:
                        e_exp2 = e_exp3 + f"{testorder[2]}={l_b}, "
                    
                    #print(f"=== Setting {testorder[2]} to {l_b}, #{b+1} of {len(loops[2])}. ===")
                    
                for c, l_c in enumerate(loops[1]):
                    if testorder[1] != None:
                    
                        if type(l_c) == str:
                            e_exp1 = e_exp2 + f"{testorder[1]}=\"{l_c}\", "
                        else:
                            e_exp1 = e_exp2 + f"{testorder[1]}={l_c}, "
                        
                        #print(f"== Setting {testorder[1]} to {l_c}, #{c+1} of {len(loops[1])}. ==")
                        
                    for d, l_d in enumerate(loops[0]):
                        if testorder[0] != None:
                        
                            if type(l_d) == str:
                                e_exp0 = e_exp1 + f"{testorder[0]}=\"{l_d}\", **kwargs)"
                            else:
                                e_exp0 = e_exp1 + f"{testorder[0]}={l_d}, **kwargs)"
                            
                            #print(f"= Setting {testorder[0]} to {l_d}, #{d+1} of {len(loops[0])}. =")
                        num = num + 1
                        print(f"image #{num}")
                        
                        print(f"evaluating {e_exp0}")    
                        output_images, seed, info, stats = eval(e_exp0)
                        print(f'Seed: {seed}')
                        print(info)
                        print(stats)
                        print()
            ...
            #    grid_file = f"grid-{grid_count:05}-{seed}_{prompt[0].replace(' ', '_').translate({ord(x): '' for x in invalid_filename_chars})[:128]}.{grid_ext}"
                grid.save(os.path.join(outpath, grid_file), grid_format, quality=grid_quality, lossless=grid_lossless, optimize=True)
                result_images = []
